...
# [POSSIBLE_ERROR] [FORCE] [FAIL_ON_WARNINGS]) - Create a target to check a
# target's sources with cppcheck and the indicated options
# add_cppcheck_sources(<target-name> [UNUSED_FUNCTIONS] [STYLE] [POSSIBLE_ERROR]
# [FORCE] [FAIL_ON_WARNINGS]) - Create a target to check standalone sources with
# cppcheck and the indicated options
#
# Requires these CMake modules: Findcppcheck
#
...
function(add_cppcheck_sources _targetname)
  if(CPPCHECK_FOUND)
    ...
    if("1.${CMAKE_VERSION}" VERSION_LESS "1.2.8.0")
      ...
      add_test(${_targetname}_cppcheck_test "${CPPCHECK_EXECUTABLE}"
               ${CPPCHECK_TEMPLATE_ARG} ${_cppcheck_args} ${_files})
    else()
      # CMake 2.8.0 and newer
      add_test(NAME ${_targetname}_cppcheck_test
...
function(add_cppcheck _name)
  ...
  if(CPPCHECK_FOUND)
    ...
    if("1.${CMAKE_VERSION}" VERSION_LESS "1.2.8.0")
      # Older than CMake 2.8.0
      add_test(${_name}_cppcheck_test "${CPPCHECK_EXECUTABLE}"
               ${CPPCHECK_TEMPLATE_ARG} ${_cppcheck_args} ${_files})
    else()
      # CMake 2.8.0 and newer
    ...
    set_tests_properties(
      ${_name}_cppcheck_test PROPERTIES FAIL_REGULAR_EXPRESSION
    ...
    add_custom_command(
      ...
      PRE_BUILD
      COMMAND ${CPPCHECK_EXECUTABLE} ${CPPCHECK_QUIET_ARG}
              ${CPPCHECK_TEMPLATE_ARG} ${_cppcheck_args} ${_includes} ${_files}
      WORKING_DIRECTORY "${CMAKE_CURRENT_SOURCE_DIR}"
...
function(add_cppcheck_indvidual _name)
  if(NOT TARGET ${_name})
    message(
      FATAL_ERROR
        "add_cppcheck given a target name that does not exist: '${_name}' !")
  ...
  if(CPPCHECK_FOUND)
    ...
    foreach(_source ${_cppcheck_sources})
      ...
      set(_file_name)
      get_filename_component(_file_name ${_source} NAME_WE)
      add_custom_target(cppcheck_${_file_name})
      set_target_properties(cppcheck_${_file_name} PROPERTIES EXCLUDE_FROM_ALL
                                                              TRUE)
      add_custom_command(
        TARGET cppcheck_${_file_name}
        PRE_BUILD
        COMMAND
          ${CPPCHECK_EXECUTABLE} ${CPPCHECK_QUIET_ARG} ${CPPCHECK_TEMPLATE_ARG}
          ${_cppcheck_args} ${_includes} ${_files}
        WORKING_DIRECTORY "${CMAKE_CURRENT_SOURCE_DIR}"
        COMMENT
          "${_file_name}_cppcheck: Running cppcheck on target ${_file_name}..."
        VERBATIM)
    endforeach()

    # if("1.${CMAKE_VERSION}" VERSION_LESS "1.2.8.0") # Older than CMake 2.8.0
    # add_test(${_name}_cppcheck_test "${CPPCHECK_EXECUTABLE}"
    # ${CPPCHECK_TEMPLATE_ARG} ${_cppcheck_args} ${_files}) else() # CMake 2.8.0
    # and newer add_test(NAME ${_name}_cppcheck_test COMMAND
    # "${CPPCHECK_EXECUTABLE}" ${CPPCHECK_TEMPLATE_ARG} ${_cppcheck_args}
    # ${_files}) endif()

    # set_tests_properties(${_name}_cppcheck_test PROPERTIES
