...
class BidirectionalBddSimple
    : public tdzdd::DdEval<E, Node<T>, Optimal_Solution<T>> {
    struct ForwardBddSimpleBidirectional
        : ForwardBddSimple<ForwardBddSimpleBidirectional, T> {
        ForwardBddSimpleBidirectional(T* _pi, int _num_jobs)
            : ForwardBddSimple<ForwardBddSimpleBidirectional, T>(_pi,
                                                                 _num_jobs){};
        explicit ForwardBddSimpleBidirectional(int _num_jobs)
            : ForwardBddSimple<ForwardBddSimpleBidirectional, T>(_num_jobs){};
        ForwardBddSimpleBidirectional()
            : ForwardBddSimple<ForwardBddSimpleBidirectional, T>(){};
    };

    struct BackwardBddSimpleBidirectional
        : BackwardBddSimple<BackwardBddSimpleBidirectional, T> {
        BackwardBddSimpleBidirectional(T* _pi, int _num_jobs)
            : BackwardBddSimple<BackwardBddSimpleBidirectional, T>(_pi,
                                                                   _num_jobs){};
        explicit BackwardBddSimpleBidirectional(int _num_jobs)
            : BackwardBddSimple<BackwardBddSimpleBidirectional, T>(_num_jobs){};
        BackwardBddSimpleBidirectional()
            : BackwardBddSimple<BackwardBddSimpleBidirectional, T>(){};
    };
   ...
   public:
    BidirectionalBddSimple(T* _pi, int _num_jobs, int _num_layer)
        : forward_evaluator(_pi, _num_jobs),
          backward_evaluator(_pi, _num_jobs),
          num_layer(_num_layer) {}

    BidirectionalBddSimple(int _num_jobs, int _num_layer)
        : forward_evaluator(_num_jobs),
          backward_evaluator(_num_jobs),
          num_layer(_num_layer) {}

    BidirectionalBddSimple()
        : forward_evaluator(), backward_evaluator(), num_layer{} {}
