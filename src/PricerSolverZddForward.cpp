...
void PricerSolverZddCycle::evaluate_nodes(double* pi, int UB, double LB) {
    ...
    // decision_diagram->getDiagram().privateEntity(); compute_labels(pi);
    ...
    //         it.child[1]->backward_label[0].get_f() - value_Fj(w +
    //         job->processing_time, job) + pi[job->job] + pi[njobs];

    //         if (LB - (double)(num_machines - 1)*reduced_cost - result > UB -
    //         1
    //         + 0.0001 && (it.calc_yes)) {
    //             it.calc_yes = false;
    //             nb_removed_edges++;
