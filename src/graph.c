...
int adjGraph_simplify(adjGraph *G) {
  ...
  CCcheck_NULL_2(tmp_weightlist, "Failed to allocate memory to tmp_weightlist");
  ...
  for (i = 0; i < G->vcount; ++i) {
    ...
    qsort(G->nodelist[i].adj, G->nodelist[i].degree, sizeof(int),
          comp_node_ids);
    new_degree =
        unify_adjlist(G->nodelist[i].adj, G->nodelist[i].degree, tmp_adjlist);
...
int adjGraph_delete_unweighted(adjGraph *G, int **new_nweights,
  ...
  CCcheck_NULL_2(new_weightlist, "Failed to allocate memory to new_weightlist");
