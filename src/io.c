...
int print_to_csv(Problem *problem) {
  ...
  if (size == 0) {
    fprintf(
        ...
        "NameInstance", "tot_real_time", "tot_cputime", "tot_lb", "tot_lb_root",
        "tot_heuristic", "tot_build_dd", "tot_pricing", "rel_error",
...
int print_to_screen(Problem *problem) {
  ...
  case no_sol:
    printf("We didn't decide if this instance is feasible or "
           "infeasible\n");
    break;
  ...
  printf("Compute_schedule took %f seconds(tot_scatter_search %f, "
