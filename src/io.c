...
int print_to_csv(wctproblem *problem) {
    ...
    if (size == 0) {
        fprintf(file,
                ...
                "global_upper_bound",
                "first_rel_error",
                "nb_generated_col", "date","nb_iterations_rvnd", "stabilization","alpha", "pricing_solver");
    }
    ...
    fprintf(file,
            ...
            date.month, date.year, parms->nb_iterations_rvnd, parms->stab_technique,parms->alpha,parms->pricing_solver);
    fclose(file);
CLEAN:
    return val;
}
