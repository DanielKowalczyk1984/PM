...
int compute_objective(NodeData *pd, Parms *parms) {
  ...
  if (pd->iterations % pd->njobs == 0) {
    printf("Current primal LP objective: %19.16f  (LP_dual-bound %19.16f, "
...
int compute_lower_bound(Problem *problem, NodeData *pd) {
  ...
  if (dbg_lvl() > 1) {
    printf("Starting compute_lower_bound with lb %d and ub %d at depth %d(id "
           "= "
           "%d, opt_track = %d)\n",
      ...
      case stab_hybrid:
        break_while_loop =
            (CC_ABS(pd->eta_out - pd->eta_in) <
             0.0001); // || (ceil(pd->eta_in - 0.00001) >= pd->eta_out);
        break;
  ...
  if (pd->iterations < pd->maxiterations &&
    ...
    case GRB_OPTIMAL:
      ...
      if (dbg_lvl() > 1) {
        printf("Found lb = %d (%f) upper_bound = %d (id= %d, "
               "iterations = "
               "%d,opt_track = %d).\n",
               pd->lower_bound, pd->LP_lower_bound, pd->upper_bound, pd->id,
               pd->iterations, pd->opt_track);
      }
