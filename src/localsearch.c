...
static int local_search_create_processing_list(Solution *sol,
  ...
  for (int i = 0; i < data->nmachines; ++i) {
    ...
    qsort(data->processing_list_1[i], njobs - l, sizeof(processing_list_data),
...
static int local_search_create_processing_list_2(Solution *sol,
  ...
  for (int i = 0; i < data->nmachines; ++i) {
    ...
    qsort(data->processing_list_2[i], njobs - l, sizeof(processing_list_data),
          compare_process_list_b);
  }
...
void local_search_forward_insertion(Solution *sol, local_search_data *data,
  ...
  if (dbg_lvl() > 0) {
    printf("forward insertion with l = %d, running time = %f and improvement "
           "%d\n",
...
void local_search_backward_insertion(Solution *sol, local_search_data *data,
  ...
  if (dbg_lvl() > 0) {
    printf("backward insertion with l = %d, running time = %f and improvement "
           "%d\n",
           l, CCutil_zeit() - runningtime, max);
    print_line();
  }
...
void local_search_swap_intra(Solution *sol, local_search_data *data, int l1,
  ...
  if (dbg_lvl() > 0) {
    printf("intra swap with l1 = %d and l2 = %d, running time = %f and "
...
void local_search_insertion_inter(Solution *sol, local_search_data *data,
  ...
  if (dbg_lvl() > 0) {
    printf("inter insertion with l = %d, running time = %f and improvement %d "
...
void local_search_swap_inter(Solution *sol, local_search_data *data, int l1,
  ...
  if (dbg_lvl() > 0) {
    printf("inter insertion with l1 = %d and l2 = %d, running time = %f and "
