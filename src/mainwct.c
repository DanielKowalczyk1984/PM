...
static void usage(char *f) {
  ...
  fprintf(stderr, "   -s int  Node selection: 0 = none, 1= minimum lower "
                  "bound(default), 2 = DFS\n");
  fprintf(stderr, "   -l dbl  Cpu time limit for branching\n");
  fprintf(stderr, "   -L dbl  Cpu time limit for heurisric construction\n");
  fprintf(stderr, "   -B int  Branch and Bound use: 0 = no branch and bound, 1 "
  ...
  fprintf(stderr, "   -Z int  Use strong branching: 0 = use strong "
...
int main(int ac, char **av) {
  ...
  if (parms->pricing_solver < dp_solver) {
    ...
    root->solver = newSolver(root->jobarray, root->nmachines,
                             root->ordered_jobs, &(problem.parms));
    CCutil_stop_timer(&(problem.tot_build_dd), 0);
    print_size_to_csv(&problem, root);
  } else {
    root->solver =
        newSolverDp(root->jobarray, root->nmachines, root->H_max, parms);
  }
