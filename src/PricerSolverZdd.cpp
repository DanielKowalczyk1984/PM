...
void PricerSolverZdd::construct_mipgraph() {
  ...
  NodeMipIdAccessor vertex_mipid_list(get(boost::vertex_degree_t(), mip_graph));
    ...
    for (size_t j = 0; j < table[i].size(); j++) {
      ...
      if (n.row() != 0) {
        for (auto &it : table[i][j].list) {
          if (n == decision_diagram->root()) {
            printf("test test test test test test ldskfs;dlkf %d\n",
                   it->get_weight());
          }
        ...
        if (n != 0) {
          number++;
          printf("test test test test test %d %ld\n", number,
                 table[i][j].list.size());
          for (auto &it : table[i][j].list) {
            it->key = add_vertex(mip_graph);
            vertex_mipid_list[it->key] = number;
  ...
  for (int i = decision_diagram->topLevel(); i > 0; i--) {
#ifndef NDEBUG
    auto job = ((job_interval_pair *)g_ptr_array_index(ordered_jobs,
                                                       ordered_jobs->len - i))
                   ->j;
#endif // NDEBUG
...
void PricerSolverZdd::init_table() {
  NodeTableEntity<NodeZdd<>> &table =
      decision_diagram->getDiagram().privateEntity();
  /** init table */
  printf("test table %ld\n", table.node(decision_diagram->root()).list.size());
...
void PricerSolverZdd::represent_solution(Solution *sol) {
  ...
  // edge_writer(g, x); ColorWriterVertex vertex_writer(g, table); std::ofstream
  // outf("solution.gv"); boost::write_graphviz(outf, g, vertex_writer,
...
int PricerSolverZdd::get_num_remove_nodes() { return nb_removed_nodes; }

int PricerSolverZdd::get_num_remove_edges() { return nb_removed_edges; }

size_t PricerSolverZdd::get_datasize() { return decision_diagram->size(); }

size_t PricerSolverZdd::get_size_graph() { return decision_diagram->size(); }

int PricerSolverZdd::get_num_layers() { return decision_diagram->topLevel(); }
...
double PricerSolverZdd::get_cost_edge(int idx) { return 0.0; }
