...
PricerSolverSimpleDp::PricerSolverSimpleDp(GPtrArray *_jobs, int _num_machines, int _Hmax) :
    PricerSolverBase(_jobs, _num_machines), Hmax(_Hmax), size_graph(0u), A(new Job* [Hmax + 1]), F(new double[Hmax + 1]) {
        init_table();
}
...
void PricerSolverSimpleDp::add_constraint(Job *job, GPtrArray *list, int order) {
