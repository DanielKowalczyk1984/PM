...
PricerSolverSimpleDp::PricerSolverSimpleDp(GPtrArray *_jobs, int _num_machines,
                                           int _Hmax)
    : PricerSolverBase(_jobs, _num_machines), Hmax(_Hmax), size_graph(0u),
      A(new Job *[Hmax + 1]), F(new double[Hmax + 1]) {
  init_table();
}
...
void PricerSolverSimpleDp::build_mip() { return; }
    ...
    for (int i = 1; i < njobs + 1; i++) {
      ...
      if (t >= job->processing_time) {
        if (F[t - job->processing_time] -
            ...
            F[t]) {
          F[t] = F[t - job->processing_time] - value_Fj(t, job) + _pi[job->job];
          A[t] = job;
        }
      }
    }
...
int PricerSolverSimpleDp::get_num_remove_nodes() { return 0; }

int PricerSolverSimpleDp::get_num_remove_edges() { return 0; }

size_t PricerSolverSimpleDp::get_datasize() { return 0u; }

size_t PricerSolverSimpleDp::get_size_graph() { return 0u; }

int PricerSolverSimpleDp::get_num_layers() { return 0; }
...
double PricerSolverSimpleDp::get_cost_edge(int idx) { return 0.0; }

bool PricerSolverSimpleDp::check_schedule_set(GPtrArray *set) { return true; }
