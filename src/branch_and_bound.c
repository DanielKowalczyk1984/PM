...
static int get_int_heap_key(double dbl_heap_key, int v1, int v2, int nmachines,
  ...
  double error2 = dbl_heap_key > error ? (ABS(error - temp) + 0.0001) / (error)
...
int insert_frac_pairs_into_heap(NodeData *pd, int *nodepair_refs,
                                double *nodepair_weights, int npairs,
                                HeapContainer *heap) {
  int val = 0;
  int i;
  int ref_key;
    ...
    if (new_lower_bound > pd->lower_bound) {
      if (!pd->parent) { /* i.e. pd == root_cd */
        ...
        printf("Lower bound increased from %d to %d (%s). \n", pd->lower_bound,
               new_lower_bound, current_timestr);
      }
...
static int collect_same_child_conflict(NodeData *cd) {
  ...
  for (c = 0; c < cd->nsame; ++c) {
    if (cd->same_children[c].nbbest &&
        (!cd->nbbest || cd->same_children[c].besttotwct < cd->upper_bound)) {
      if (cd->nbbest) {
...
static int collect_diff_child_conflict(NodeData *cd) {
  ...
  for (c = 0; c < cd->ndiff; ++c) {
    if (cd->diff_children[c].nbbest &&
        (!cd->nbbest || cd->diff_children[c].besttotwct < cd->upper_bound)) {
      if (cd->nbbest) {
        schedulesets_free(&(cd->bestcolors), &(cd->nbbest));
      }
...
int insert_into_branching_heap(NodeData *pd, Problem *problem) {
  ...
  if (dbg_lvl()) {
    printf("Inserting into branching heap with lb %d and ub %d at depth %d "
           "(id "
           "= %d) heap_key = %d\n",
