 ...
 *
 * Return -1, 0, 1 if first GUB has larger, same, smaller weight than the second GUB.
 */
int compare_GUBpair(
   const void* a,                            /**< first GUB */
...
/* Separates GUB facet inequalities for a knapsack polytope with knapsack inequality
 * 2 * (x_1 + ... + x_r) + (y_1 + ... + y_s) - (z_1 + ... + z_t) <= rhs,
 * where some of the y- and z-variables are contained in GUB constraints.
 *
 * To avoid handling y- and z-variables that are not contained in a GUB, we assume
 * that also single variables can form a GUB.
 *
 * We encode GUBs in the following way:
 *
 * - posvals contains the values of variables with a +1 coefficient in the knapsack inequality
 * - posgubs is a list containing encoding the GUBs
 * - posgubsbegins encodes where the GUBs in posgubs begin and end
 *
 * for example,
 *
 * if posgubs = [0,3,2,4,1] and posgubsbegins = [0,2,5], then posgubs contains two GUBs, the first one
 * is formed by the entries posgubsbegins[0], ..., posgubsbegins[1] - 1 in posgubs, i.e., {0,3}
 * and the second one corresponds to the entries between posgubsbegins[1], ..., posgubsbegins[2] - 1, i.e., {2,4,1}.
 *
 * The weights of the GUBs are then posvals[posgubs[posgubsbegins[0]]] + ... + posvals[posgubs[posgubsbegins[1] - 1]]
 * and posvals[posgubs[posgubsbegins[1]]] + ... + posvals[posgubs[posgubsbegins[2] - 1]].
 */
void separateGUBknapsackFacets(
   double*     twovals,                      /**< values of solution to separate for variables with 2-coefficient */
   int         ntwovals,                     /**< number of variables with 2-coefficient */
   double*     posvals,                      /**< values of solution to separate for variables with 1-coefficient */
   int         nposvals,                     /**< number of variables with 1-coefficient */
   int*        posgubs,                      /**< array containing indices of the GUBs in posvals */
   int*        posgubsbegins,                /**< array containing the start positions of GUBs in posgubs */
   int         nposgubs,                     /**< number of GUBs encoded in posgubs */
   double*     negvals,                      /**< values of solution to separate for variables with (-1)-coefficient */
   int         nnegvals,                     /**< number of variables with (-1)-coefficient */
   int*        neggubs,                      /**< array containing indices of the GUBs in negvals */
   int*        neggubsbegins,                /**< array containing the start positions of GUBs in neggubs */
   int         nneggubs,                     /**< number of GUBs encoded in neggubs */
   int         rhs,                          /**< right-hand side of knapsack inequality */
   int**       twocoeffs,                    /**< pointer to coefficients of 2-variables in separated inequality */
   int**       poscoeffs,                    /**< pointer to coefficients of 1-variables in separated inequality */
   int**       negcoeffs,                    /**< pointer to coefficients of (-1)-variables in separated inequality */
   ...
   )
{
   ...
   *success = false;
    ...
    * (all these variables will have a 1-coefficients in an inequality and thus contribute)
    */
   for (i = 0; i < ntwovals; ++i)
      sumtwovals += twovals[i];

   /* for each GUB in posgubs, store its total weight and index (i for the i-th GUB) */
   posgubweightinfo = (struct GUBinfo*) malloc(nposgubs * sizeof(struct GUBinfo));
   ...
   /* for each GUB in neggubs, store its total negated weight and index ( - (i + 1) for the i-th GUB) */
   neggubweightinfo = (struct GUBinfo*) malloc(nneggubs * sizeof(struct GUBinfo));
   ...
   /* adapt the right-hand side of the knapsack inequality by negating (-1)-coefficient GUBs */
   rhsmod = rhs + nneggubs;
   ...
   allgubweightinfo = (struct GUBinfo*) malloc(nallgubs * sizeof(struct GUBinfo));
   cnt = 0;
   cnt1 = 0;
   cnt2 = 0;
   for (cnt = 0; cnt < nallgubs; ++cnt)
   {
      if ( cnt1 == nposgubs || posgubweightinfo[cnt1].gubweight < neggubweightinfo[cnt2].gubweight )
      {
      ...
      if ( facetlhs > facetrhs )
      {
         ...
         *negcoeffs = (int*) malloc(nnegvals * sizeof(int));
         ...
         for (i = 0; i <= r; ++i)
         {
            ...
            if ( index >= 0 )
            {
               for (j = posgubsbegins[index]; j < posgubsbegins[index + 1]; ++j)
            ...
            else
            {
               ...
               for (j = neggubsbegins[index]; j < neggubsbegins[index + 1]; ++j)
