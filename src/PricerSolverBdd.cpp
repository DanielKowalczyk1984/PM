...
PricerSolverBdd::PricerSolverBdd(GPtrArray *_jobs, int _num_machines,
    ...
    : PricerSolverBase(_jobs, _num_machines, _ordered_jobs), size_graph(0),
      nb_removed_edges(0), nb_removed_nodes(0), env(new GRBEnv()),
...
void PricerSolverBdd::build_mip() {
  try {
    ...
    NodeTableEntity<> &table = decision_diagram->getDiagram().privateEntity();
    IndexAccessor vertex_index_list(get(boost::vertex_index_t(), mip_graph));
    NodeIdAccessor vertex_nodeid_list(get(boost::vertex_name_t(), mip_graph));
    EdgeTypeAccessor edge_type_list(get(boost::edge_weight_t(), mip_graph));
    ...
    for (auto it = edges(mip_graph); it.first != it.second; it.first++) {
      if (edge_type_list[*it.first]) {
        ...
        edge_var_list[*it.first].x =
            model->addVar(0.0, (double)num_machines, 0.0, GRB_CONTINUOUS);
      }
    }
...
void PricerSolverBdd::disjunctive_inequality(double *x, Solution *sol) {
  ...
  try {
    ...
    for (auto it = edges(mip_graph); it.first != it.second; it.first++) {
      edge_var_list[*it.first].alpha =
          model_ineq->addVar(-GRB_INFINITY, GRB_INFINITY,
                             p[edge_index_list[*it.first]], GRB_CONTINUOUS);
    }

    for (auto it = vertices(mip_graph); it.first != it.second; it.first++) {
      node_var_list[*it.first].omega[0] =
          model_ineq->addVar(-GRB_INFINITY, GRB_INFINITY, 0.0, GRB_CONTINUOUS);
      node_var_list[*it.first].omega[1] =
          model_ineq->addVar(-GRB_INFINITY, GRB_INFINITY, 0.0, GRB_CONTINUOUS);
    }
    ...
    GRBVar alpha =
        model_ineq->addVar(-GRB_INFINITY, GRB_INFINITY, -1.0, GRB_CONTINUOUS);
    model_ineq->update();
    /**
     ...
     */
    std::unique_ptr<GRBConstr[]> constrs0(model_ineq->addConstrs(
        constraints_0.get(), sense.get(), rhs.get(), nullptr, num_edges + 1));
    std::unique_ptr<GRBConstr[]> constrs1(model_ineq->addConstrs(
        constraints_1.get(), sense.get(), rhs.get(), nullptr, num_edges + 1));
...
int PricerSolverBdd::get_num_remove_nodes() { return nb_removed_nodes; }

int PricerSolverBdd::get_num_remove_edges() { return nb_removed_edges; }

size_t PricerSolverBdd::get_datasize() { return decision_diagram->size(); }

size_t PricerSolverBdd::get_size_graph() { return decision_diagram->size(); }

int PricerSolverBdd::get_num_layers() { return decision_diagram->topLevel(); }
...
double PricerSolverBdd::get_cost_edge(int idx) { return 0.0; }
