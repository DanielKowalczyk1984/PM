...
int solve_stab(NodeData *pd, wctparms *parms) {
  ...
  do {
    k += 1.0;
    alpha = pd->hasstabcenter ? CC_MAX(0, 1.0 - k * (1.0 - pd->alpha)) : 0.0;
  ...
  if (pd->iterations % pd->njobs == 0) {
    printf("alpha = %f, result of primal bound and Lagragian "
           "bound: out =%f, in = %f\n",
...
int solve_stab_dynamic(NodeData *pd, wctparms *parms) {
  ...
  do {
    ...
    if (pd->reduced_cost >= 0.000001) {
      compute_subgradient(sol, pd);
      adjust_alpha(pd->pi_out, pd->pi_in, pd->subgradient_in, pd->njobs, alpha);
      val = construct_sol(pd, &sol);
      CCcheck_val_2(val, "Failed in construct_sol_stab");
      pd->alpha = alpha;
      pd->update = 1;
  ...
  if (pd->iterations % pd->njobs == 0) {
    printf(" alpha = %f, result of primal bound and Lagragian bound: out =%f, "
           "in = %f\n",
           pd->alpha, pd->eta_out, pd->eta_in);
  }
