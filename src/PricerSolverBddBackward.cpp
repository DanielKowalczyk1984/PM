...
PricerSolverBddBackwardSimple::pricing_algorithm(double *_pi) {
...
void PricerSolverBddBackwardSimple::evaluate_nodes(double *pi, int UB,
  ...
  for (int i = decision_diagram->topLevel(); i > 0; i--) {
    for (auto &it : table[i]) {
      ...
      Job *job = it.get_job();
      double result =
          it.forward_label[0].get_f() + it.child[1]->backward_label[0].get_f() -
          value_Fj(w + job->processing_time, job) + pi[job->job] + pi[njobs];
...
OptimalSolution<double>
PricerSolverBddBackwardCycle::pricing_algorithm(double *_pi) {
  evaluator.initializepi(_pi);
  return decision_diagram->evaluate_backward(evaluator);
}
    ...
    for (auto &it : table[i]) {
      ...
      if (it.forward_label[0].get_previous_job() != job &&
        ...
        double result = it.forward_label[0].get_f() +
                        ...
                        value_Fj(w + job->processing_time, job) + pi[job->job] +
                        pi[njobs];
        if (LB - (double)(num_machines - 1) * reduced_cost - result >
                UB + 0.0001 &&
            (it.calc_yes)) {
