...
PricerSolverArcTimeDp::PricerSolverArcTimeDp(GPtrArray *_jobs,
    ...
    : PricerSolverBase(_jobs, _num_machines), Hmax(_Hmax), n(_jobs->len),
      size_graph(0u), vector_jobs() {
  for (int i = 0; i < n; ++i) {
...
void PricerSolverArcTimeDp::init_table() {
  ...
  for (int i = 0; i < n - 1; ++i) {
    ...
    for (int j = i + 1; j < n; ++j) {
      ...
      for (int t = tmp_i->processing_time; t <= Hmax - tmp_j->processing_time;
           ++t) {
        if (delta1(i, j, t) >= 0) {
          remove_arc(i, j, t);
          size_graph--;
...
void PricerSolverArcTimeDp::build_mip() { return; }
  ...
  while (T > 0) {
    ...
    if (aux_job != n) {
      ...
      sol.cost += value_Fj(aux_T + vector_jobs[aux_job]->processing_time,
                           vector_jobs[aux_job]);
      sol.obj +=
          _pi[aux_job] - value_Fj(aux_T + vector_jobs[aux_job]->processing_time,
                                  vector_jobs[aux_job]);
    }
    job = aux_job;
    T = aux_T;
...
int PricerSolverArcTimeDp::get_num_remove_nodes() { return 0; }

int PricerSolverArcTimeDp::get_num_remove_edges() { return 0; }

size_t PricerSolverArcTimeDp::get_datasize() { return 0u; }

size_t PricerSolverArcTimeDp::get_size_graph() { return 0u; }

int PricerSolverArcTimeDp::get_num_layers() { return 0; }
...
double PricerSolverArcTimeDp::get_cost_edge(int idx) { return 0.0; }

bool PricerSolverArcTimeDp::check_schedule_set(GPtrArray *set) { return true; }
