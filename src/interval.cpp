...
void g_print_interval(gpointer data, MAYBE_UNUSED gpointer user_data) {
    interval* a = static_cast<interval*>(data);
    fmt::print("interval {}: ({}, {}]: ", a->key, a->a, a->b);
...
gint g_compare_interval_data(gconstpointer a, gconstpointer b, gpointer data) {
    ...
    interval*  user_data = static_cast<interval*>(data);
    int        diff = user_data->b - user_data->a;
    double     w_x = (x->due_time >= user_data->b)
                         ? 0.0
                         : static_cast<double>(x->weight) / x->processing_time;
    double     w_y = (y->due_time >= user_data->b)
                         ? 0.0
                         : static_cast<double>(y->weight) / y->processing_time;
...
void interval_init(interval*  p,
    ...
    while (p->b - p->a <= j->processing_time &&
           p->begin < static_cast<int>(jobarray->len)) {
...
gpointer g_copy_interval(gconstpointer src, gpointer data) {
    ...
    const interval* src_interval = static_cast<const interval*>(src);
...
gpointer g_copy_interval_pair(gconstpointer src, gpointer data) {
    ...
    const job_interval_pair* pair = static_cast<const job_interval_pair*>(src);
...
void g_interval_free(void* p) {
    if (p != NULL) {
        interval* part = static_cast<interval*>(p);
...
void interval_pair_free(void* p) {
    if (p != NULL) {
        interval_pair* tmp = static_cast<interval_pair*>(p);
...
void print_interval_pair(GPtrArray* ordered_jobs) {
    ...
    for (size_t i = 0; i < ordered_jobs->len; i++) {
        ...
        if (tmp_p->I != cur) {
            cur = tmp_p->I;
            fmt::print("\n");
            fmt::print("Interval {} ({},{}]: ", cur->key, cur->a, cur->b);
        }
        fmt::print("%d ", tmp_p->j->job);
    }
    fmt::print("\n");
}
