  ...
  *strongest_v2 = -1;
  ...
  case use_strong_branching:
    while ((min_nodepair = (int *)heapcontainer_min(cand_heap)) &&
      ...
      if (dbg_lvl() > 0) {
        printf("Creating branches for v1 = %d, v2 = %d (node-pair "
               "weight %f)\n",
      ...
      dbl_child_lb =
          (same_children->LP_lower_bound < diff_children->LP_lower_bound)
      ...
      if (dbg_lvl() > 1) {
        printf("Found child bound of %f for v1 = %d, v2 = %d, "
               "nodepair_weight "
               "= %f .\n",
               dbl_child_lb, v1, v2,
  ...
  case no_strong_branching:
    ...
    if (dbg_lvl() > 0) {
      printf("Creating branches for v1 = %d, v2 = %d (node-pair weight "
             "%f)\n",
             v1, v2, nodepair_weights[(int)(min_nodepair - nodepair_refs)]);
    }
...
int create_branches_conflict(NodeData *pd, Problem *problem) {
  ...
  val = insert_frac_pairs_into_heap(pd, nodepair_refs, nodepair_weights, npairs,
                                    heap);
