...
function runBenchmark(){
    instances=($(ls $HOME/Dropbox/PapersOR/PM/implementation/instances/wt{040,050,100}/*{1,6}.dat))
    machines=(2 4)
    solvers=($(seq 10))
    printf "%s\n" "${instances[@]}" | xargs -I{} printf "{} %s\n" "${machines[@]}" | xargs -I{} printf "-a %s {}\n" "${solvers[@]}" | parallel --no-notice -P 6 --eta --colsep ' ' "build/PMRelease -f 0 -S 1 {}";
}
